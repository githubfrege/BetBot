    ...
    public static class BotOperations
    {
        ...
        public static int WhatToBet(double odds, int tableChips, int chipOffer, int myChips, int bankRoll, out double expectedValue)
        {
            ...
            for (int i = chipOffer; i <= myChips + chipOffer; i++)
            {
                ...
                double ror = getRiskOfRuin(odds, i, bankRoll);
                if (currentEV > maxEv && (ror < 0.02))
                {
                    maxEv = currentEV;
                    bet = i;
                }
        ...
        public static double GetOdds(List<KeyValuePair<int, int>> holeCardsKvpList, List<KeyValuePair<int, int>> tableKvpList)
        {
            ...
            Parallel.ForEach(HandEvaluation.CardCombos(availableCards.ToIEnum(), 2), villainHoles => //every combination of cards our opponent may have
            {
                //Debug.WriteLine("vhn" + villainHoleNum);
                ...
                if (tableKvpList.Count < 5)
                {
                    foreach (ulong cardAdditions in HandEvaluation.CardCombos(currentAvailableCards.ToIEnum(), 5 - tableKvpList.Count)) //all combinations of cards that may be added to the existing table
                    {
                        ...
                        Interlocked.Exchange(ref heroWins, heroWins + scoreUpdate.heroPoints);
                        Interlocked.Exchange(ref villainWins, villainWins +scoreUpdate.villainPoints);
        ...
        public static (int mainScore, long tieBreaker) HandToPlay(ulong holes, ulong cardsOnTable)
        {
            ...
            foreach (ulong combo in CardCombos(cardsOnTable.ToIEnum(), 3))
            {
                ...
                if ((currentScore.mainScore.CompareTo(max.mainScore) != 0 ? currentScore.mainScore.CompareTo(max.mainScore) : currentScore.tieBreaker.CompareTo(max.tieBreaker)) > 0)
                {
                    ...
                    object currentScoreObject = currentScore;

                    Interlocked.Exchange(ref maxObject, currentScoreObject);
                    max = currentScore;
                }
